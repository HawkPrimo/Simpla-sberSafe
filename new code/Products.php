...
class Products extends Simpla
{
    ...
    public function get_products($filter = array())
    {
        ...
        /*multioption_filter_price_f_slider*/
        $price_filter_join = '';
        $price_filter_where = '';
        //for features slider
        $features_slider_filter = '';
        /*/multioption_filter_price_f_slider*/
        ...
        if(isset($filter['google']))
            $google_filter = $this->db->placehold('inner join __variants v on v.product_id=p.id and v.google=?', intval($filter['google']));
        /*/upload_to_google*/

        /*upload_to_mailru*/
        if(isset($filter['mailru']))
            $mailru_filter = $this->db->placehold('inner join __variants v on v.product_id=p.id and v.mailru=?', intval($filter['mailru']));
        /*/upload_to_mailru*/
        ...
        /*multioption_filter_price_f_slider*/
        //for features slider
        if (!empty($filter['features_slider'])) {
            foreach($filter['features_slider'] as $feature=>$value) {
                $features_slider_filter .= $this->db->placehold('AND p.id in (SELECT product_id FROM __options WHERE feature_id=? AND value >= ? and value <= ? ) ', $feature, intval($value['min']), intval($value['max']));
            }
        }
        /*/multioption_filter_price_f_slider*/
        ...
        $query = "SELECT  
                ...
                /*multioption_filter_price_f_slider*/ $price_filter_join /*/multioption_filter_price_f_slider*/
				WHERE 
					1
					$product_id_filter
					$brand_id_filter
    ...
    public function update_product($id, $product)
    {
        ...
        $queryString = self::getQueryString($product);

        if($this->db->query("UPDATE __products SET " . $queryString ." WHERE id=?", $id)){
            return $id;
        }else{
            return false;
        }
    ...
    private static function getQueryString($product)
    {
        $queryString = "h1_name='{$product['name']}', " .
                       "annotation='{$product['annotation']}', " .
                       "meta_title='{$product['meta_title']}', " .
                       "meta_keywords=''";

        if(isset($product['body'])){
            $queryString = $queryString . ", body='{$product['body']}'" .", meta_description='{$product['meta_description']}'";
        }

        if (isset($product['brand_id'])){
            $queryString = $queryString . ", brand_id='{$product['brand_id']}'";
        }
    ...
    */
    public function delete_product($id)
    {
        if(!empty($id))
        {
            ...
            /*accessories*/
            $accessories = $this->get_accessories($id);
            foreach($accessories as $accessory)
                $this->delete_accessory($id, $accessory->related_id);
            /*/accessories*/
            ...
            //Очищаем ссылки для парсинга
            $query = $this->db->query("UPDATE __parsers_pages SET product_id=NULL WHERE product_id=?", $id);
    ...
    public function duplicate_product($id)
    {
        ...
        foreach($categories as $k=>$c)
            $this->categories->add_product_category($new_id, $c->category_id, $k);
        ...
        $accessories = $this->get_accessories($id);
        foreach($accessories as $accessory)
            $this->add_accessory($new_id, $accessory->related_id);
        /*/accessories*/
    ...
    /*accessories*/
    function get_accessories($product_id = array())
    {
        if(empty($product_id))
            return array();
    ...
    public function add_accessory($product_id, $related_id, $position=0)
    {
        $query = $this->db->placehold("INSERT IGNORE INTO __accessories SET product_id=?, related_id=?, position=?", $product_id, $related_id, $position);
        $this->db->query($query);
        return $related_id;
    }

    public function delete_accessory($product_id, $related_id)
    {
        $query = $this->db->placehold("DELETE FROM __accessories WHERE product_id=? AND related_id=? LIMIT 1", intval($product_id), intval($related_id));
        $this->db->query($query);
    }
    /*/accessories*/
    ...
    */
    ...
    /*multioption_filter_price_f_slider*/
    public function get_max_price($filter = array())
    {
        $limit = 100;
        $page = 1;
        $category_id_filter = '';
    ...
    public function get_min_price($filter = array())
    {
        $limit = 100;
        $page = 1;
        $category_id_filter = '';
        ...
        if(isset($filter['limit']))
            $limit = min(1, intval($filter['limit']));

        if(isset($filter['page']))
            $page = min(1, intval($filter['page']));
